...
# Automation call
# from core.common import s3FileListing ,storeResponse,move_s3_object
# from core.constant import TRANSCRIBE_PARAM,S3PATH

# self call
from common import initTranscriptEngine, s3FileListing, storeResponse, move_s3_object, df_to_excel_s3_upload
from constant import TRANSCRIBE_PARAM, S3PATH, AUDIO_EXT
...
def transcriptRequest(url, payload):
    response = requests.post(url=url, data=payload, headers={
                             'Content-Type': 'application/json'}, timeout=(10, 600))
    return response


def transcribe(**config_prm):
    url = config_prm["transcribe_url"]
    ...
    logs = []
    k = 0
    log = {"status": "", "count": 0, "status_code": "", "status_message": "", 'lang': "en",
           "transcribed_detected_lang": "", "ext": "", "api_version": "", "confidence_score": "", "filepath": "", "json": ""}
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if respObj["status_code"] == 0 and respObj["confidence_score"] >= 0.2:
                        log = {"status": 'scussess', "count": k, "status_code": respObj["status_code"], "status_message": respObj["status_message"], 'lang': param["lang"], "transcribed_detected_lang": respObj[
                        ...
                        filename = audio_filename.split(
                            ".wav")[0].strip()+".json"
                        prefix = f"{s3_transcript}json/{filename}"
                        log['json'] = storeResponse(
                            bucket_name, prefix, respObj["transcribed_text"])  # json store
                        filename_txt = audio_filename.split(
                            ".wav")[0].strip()+".txt"
                        prefix_txt = f"{s3_transcript}txt/{filename_txt}"
                        storeResponse(bucket_name, prefix_txt,
                                      respObj["transcribed_text"])  # txt store
                        # move processed file
                        move_s3_object(bucket_name, path,
                                       f"{s3_processed}{audio_filename}")
                        time.sleep(60)
                    else:
                        status, respObj = try_failure(
                            response.json(), url, json_param)
                        if status != False:
                            # respObj = response.json()
    ...
    df_fn = "GPU_Transcription_"+cdt.strftime("%Y_%m_%d_%H%M%S")
